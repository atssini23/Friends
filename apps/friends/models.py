...
class UserManager(models.Manager):
    ...
    def addFriend(self, user_id, friend_id):
        ...
        Friend.objects.create(bestfriend=user, secondfriend=friend)
        Friend.objects.create(bestfriend=friend, secondfriend=user)

    def removeFriend(self, user_id, friend_id):
        user = self.get(id=user_id)
        friend = self.get(id=friend_id)
        friendship1 = Friend.objects.get(bestfriend=user, secondfriend=friend)
        friendship2 = Friend.objects.get(bestfriend=friend, secondfriend=user)
        friendship1.delete()
        friendship2.delete()
...
class Friend(models.Model):
    bestfriend = models.ForeignKey(User, related_name='requester')
    secondfriend = models.ForeignKey(User, related_name='accepter')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
